...
**Стандартний прогін — ЗАВЖДИ два скріни: світлий і темний** (рішення Yuriy 28.08.2026):

```bash
GRAFIK_TOKEN=$T node shot.mjs /workers/66 light.png
DARK=1 GRAFIK_TOKEN=$T node shot.mjs /workers/66 dark.png
```

## Крок 3 — ДИВИСЬ і ітеруй

1. Прочитай ОБИДВА PNG (Read) і чесно перевір: тексти не налазять і не обрізаються, дати не ламаються на два рядки, порожні стани компактні, скрол-контейнери працюють, `console errors: none`.
2. У темному окремо: hex-хардкоди, що не перетемнились (біла пляма/чорний текст на темному), нечитабельні бейджі `bg-*-50`, інлайн-стилі кольорів — за правилами CLAUDE.md дарк іде через CSS-змінні, тож будь-який «світлий острів» = баг верстки.
3. Знайшов дефект → правка → `pnpm --filter @workspace/web run build` → нова пара скрінів. Дві-три ітерації — норма.
4. Для порівняння варіантів дизайну з користувачем — вбудуй скріни (через `sips --resampleWidth 1100` + base64) в Artifact.
