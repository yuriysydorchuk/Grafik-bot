...
for (const [, x] of onlyOrig.sort((a, b) => b[1].pay - a[1].pay))
...
for (const [, x] of onlySite.sort((a, b) => b[1].pay - a[1].pay))
  console.log(`  ${x.name} — ${r2(x.hours)} год, ${r2(x.pay)} zł (${[...x.factories].join(",")})`);
...
for (const d of diffs.filter(d => Math.abs(d.dPay) >= 1).sort((a, b) => Math.abs(b.dPay) - Math.abs(a.dPay)))
...
for (const d of diffs.filter(d => Math.abs(d.dHours) >= 0.5).sort((a, b) => Math.abs(b.dHours) - Math.abs(a.dHours)))
...
for (const d of diffs.filter(d => Math.abs(d.dPay) < 1 && (Math.abs(d.dKonto) >= 1 || Math.abs(d.dGot) >= 1)).sort((a, b) => Math.abs(b.dKonto) - Math.abs(a.dKonto)))
