// Разово: кари/одяг/komornik/kaucja травня з оригінальної люблінської сводної
// → клітинки сайтової сводної (як ручний ввід), ЛИШЕ Люблін.
// Запуск: node --env-file=../../.env --import ./test-hooks.mjs import-may-lublin-kary.mjs
...
for (const [t, rows] of g) {
  ...
  for (const src of p.rows) {
    const core = { kara: src.kara, komornik: src.komornik, odziez: src.odziez, kaucja: src.kaucja };
    const ex = { karaEs: src.extras.karaEs, karaKlient: src.extras.karaKlient };
    ...
    await db.update(svodniRowsTable).set({
      kara: merged.kara, komornik: merged.komornik, odziez: merged.odziez, kaucja: merged.kaucja,
