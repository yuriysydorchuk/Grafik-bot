...
const siteRows = (await db.select().from(svodniRowsTable).where(eq(svodniRowsTable.periodMonth, MONTH))).filter(r => r.city === "Люблін");
...
for (const [name, kind] of [["lublin-2026-05","L"]]) {
