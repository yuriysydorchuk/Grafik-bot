...
import {
  ...
  legalStatusOf, computeKsiegHours, applyLegalDefaults, computePayout,
} from "./svodni.ts";
...
test("Люблін: Premia ES — бонус/год до базової ставки; вшитий у ставку бонус дає ⚠", () => {
  const rows = [
    ...
    // ставка вже містить бонус (≥ 25,35 + 1,5) — бонус не додається вдруге
  ...
  assert.equal(p.rows[1]!.mismatch, null, "вшита ставка: бонус не подвоюється");
});
