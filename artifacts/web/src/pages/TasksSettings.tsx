            ...
            <tbody>
              {data.rules.map(r => (
                <tr key={r.code} className="border-t border-slate-50">
                  ...
                  <td className="py-1.5 pr-2">{r.leadDays == null && !["doc_expiring", "contract", "absence_unexplained", "candidate_stale"].includes(r.code) ? <span className="text-xs text-slate-400">{t("одразу")}</span> : <input type="number" min={0} defaultValue={r.leadDays ?? ""} disabled={!canManage} onBlur={e => upd.mutate({ code: r.code, body: { leadDays: e.target.value === "" ? null : Number(e.target.value) } })} className="w-16 rounded border border-slate-200 px-1.5 py-1 text-xs" placeholder={r.code === "doc_expiring" ? t("з типу") : ""} />}</td>
                  <td className="py-1.5 pr-2">{admSel(r.fallbackAdminId, v => upd.mutate({ code: r.code, body: { fallbackAdminId: v } }), !canManage)}{r.scheduler && <span className="ml-1 text-[10px] text-slate-400">{t("(спершу графікова фабрики)")}</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
