...
- **Міграції — вручну через `psql`.** `drizzle-kit push` ненадійний у non-TTY; зміни накатуються SQL-командами (`ALTER TABLE ... ADD COLUMN IF NOT EXISTS ...`). Тримай SQL та схему синхронними. **Порядок накату = алфавіт назв файлів** (CI і `/deploy` роблять `for m in deploy/migrations/*.sql`): файл, що змінює таблицю з «пізнішого» файла, впаде на проді — дата в назві має йти після залежності. Перед великим батчем — сухий прогін на копії прод-дампу. Дані-сіди (реквізити фірм, шаблони) теж ідуть міграціями, ідемпотентно.
