...
- **Архів фактур на Google Drive** (`services/invoiceArchive.ts`): гілки `Faktury kosztowe` / `Faktury sprzedażowe` → рік → `M<міс>.<рр>` → фірма; імʼя файла = номер фактури. KSeF-фактури зберігаються **парою**: стандартний XML з KSeF (юридичний оригінал) + **PDF-візуалізація поряд** (`services/ksefPdf.ts`, за зразком стандартних візуалізацій KSeF: шапка/сторони/позиції/VAT/оплата/QR верифікації; шрифти DejaVu в `assets/fonts/`, лінк веб-панелі веде на PDF — рішення 13.08.2026). Скани/ручні — PDF (фото загортається через `imagePdf.ts`). KSeF API PDF **не віддає** — тільки XML; візуалізацію не «спрощувати» до сирих полів. Рядок без файла на Drive несе `drive_error` і світиться червоним на /cost-invoices — не «лагодити» приховуванням.
