...
- **Статус для виплат — лише через `services/effectiveStatus.ts`** (рішення 06.09.2026): **У сводній — ЗА МІСЯЦЬ рядка** (20.09.2026, `services/legalityMonth.ts` — єдиний дозволений міст payroll→движок): документи на кінець місяця, вісь «умова» зелена, якщо підписана умова перекриває хоч один день місяця (умова 11–21.08 → серпень legal, вересень — ні); порожній снапшот (`legal_source=none`) добирається при повторному «У сводну».) повністю оформлений за документами → статус із кешу `worker_legality` (source `documents`), інакше ручне `workers.legal_status` (`manual`) / порожнє (`none`). Payroll-код (сводна, /hours, /workers) бере `effectiveView(w, cache)` і далі читає `w.legalStatus/isStudent` як раніше; движок легальності `workers.legal_status` **не пише** і payroll-код движок **не імпортує** (гард `legality.guard.test.ts`). Сводна знімає снапшот у `svodni_rows.legal_status/legal_source` на момент формування рядка; зміна за документами йде в журнал `worker_changes` (`effectiveLegalStatus`) і застосовується лише після «Прийняти» в профілі або ревʼю при розлоку.
...
- **Виповідзення блокує графік з дати звільнення** (20.09.2026): єдиний предикат `lib/termination.ts` `terminatedOn(w, factoryId, date)` (дзеркало `web/src/lib/termination.ts`) — генератор, `POST /schedule/entry` (400), «заповнити як цей день» (reason `terminated`), веб-пікер «Додати людину». Нова поверхня, що ставить людину в графік, мусить його викликати; `is_active` сам по собі недостатній (до дати людина активна — інцидент Oleksiiuk 15.09.2026).
- **Задачі: «взято в план» сильніше за `dueAt`** — лічильники/«Мій день»/дайджести/бот-списки рахують день через `effectiveDay(due, planned, today)` з `services/tasks.ts` (план на майбутнє = не прострочено). Не додавати нових підрахунків по голому `due_at`.
- **Powiadomienie UA — по кожній фірмі-роботодавцю** (20.09.2026): обовʼязок движка має `params.companyId/company`, документ рахується лише з `employer_company_id` цієї фірми (`employerOk==="ok"`; без фірми = невідомий, не закриває); рядок UA-задачі = людина×фірма (`uaKey`, `services/uaNotification.ts`), слот Powiadomienie у профілі — по роботодавцях, коли їх >1. Громадяни ЄС (`eu_other`/`romania`) для виплат = `polak`. Karta pobytu: `attrs.article` — стаття decyzji з довідника `services/stayArticles.ts` (дзеркало web); мета виводиться зі статті, право на працю — `articleGrantsWork` (лише `verified`) або анотація «dostęp do rynku pracy»; статті з `employerBound` вимагають `employer_company_id` на документі.
- **Листування щодо пропусків — `absence_messages`** (`services/absenceMessages.ts`): офіс пише з `/absences` («Написати», «Нагадати про невиправдані» — по одному повідомленню на людину, office-рядок kind=reminder на кожен пропуск); повідомлення офісу відкриває працівнику «вікно» повторного пояснення й ще одного файлу (`workerReplyWindow`); поза вікном одноразовість первинного пояснення лишається. Телефон працівника живе ЛИШЕ в анкеті (`worker_questionnaires.phone`, у `workers` колонки немає); формат рахунків — `fmtIban` з `lib/iban.ts` обох пакетів.

- **Excel-графік клієнту позначає «(nowy)»** (21.09.2026) біля імені дрібним червоним — ЛИШЕ перший робочий день людини на ЦІЙ фабриці (`drive.ts` `loadFirstDatesAtFactory`: фактична зміна з затвердженого/розісланого тижня ∪ `factory_hours.days` ∪ записи тижня, що експортується). Нова експортна поверхня графіку мусить використовувати той самий хелпер. **Лист роботодавцю про виповідзення** — один шаблон у `settings` (`email_tpl_termination_*`, Налаштування → Email-шаблони, `services/email.ts` `renderTerminationEmail`), пропонується модалкою одразу після збереження дати виповідзення в профілі й кнопкою «лист роботодавцю»; адреси — отримувачі фабрики; факт — `worker_changes.terminationEmail`.
- **«Доїжджає сам» — ПО ФАБРИЦІ й ПОДЕННО** (рішення 11.09.2026): таблиця `worker_self_transport` (інтервали `[since, until)` пари працівник+фабрика), єдиний резолвер — `services/selfTransport.ts` (`isSelfOn(map, workerId, factoryId, date)`, `selfPredicateForWeek(weekId)`); мутації — `setSelfTransport`/`moveSelfTransportSince`, не прямі INSERT. Легасі `workers.self_transport(_since)` **не читати й не писати**. Зміна основної фабрики режим не переносить. Операційне поле графікової (гейт `editData`). Self-люди в self-день **виключаються з водійського флоу**: не в списку посадки (`bot/index.ts`), **ніколи не auto-`absent`** при підтвердженні посадки, не рахуються у лічильниках «до забрання» (`GET /driver-board` + `services/pickupGaps.ts` + пресмінне водієві у `scheduler.ts` — усі місця фільтрують через предикат по даті запису). Явку/відсутність їм ставить **графікова вручну** у веб-графіку. Свій пресмінний пуш працівник **отримує** (він на зміні). В Excel-графіку клієнту — лишаються. Генерація знять за довіз для пари зі self-днями в місяці рахує поденно (self-день — посадка водія, день «возить фірма» — явка), а не з годин сводної; перед-звірка перенесення такі пари пропускає.
