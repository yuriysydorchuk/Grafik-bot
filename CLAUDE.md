...
- **Ролі та доступи — динамічні.** Ролі веб-панелі живуть у БД-таблиці `roles` (`pages` jsonb + `caps` jsonb; системні `owner`/`scheduler`/`driver` + кастомні). Каталоги capability/сторінок продубльовані: `artifacts/api-server/src/lib/roles.ts` ↔ `artifacts/web/src/lib/roles.ts` — тримати синхронними. Гейти API — `requireCap`/`requireAnyCap` (`editData`, `viewFinance`, `assignDrivers`, `deleteWorkers`, `svodni`, `svodniSensitive`); `owner` — незмінний суперюзер (повний доступ у коді, незалежно від рядка в БД). Ролі/користувачів редагує **лише головний адмін** (`admins.is_main`, Yuriy id=1, `requireMainAdmin`); у бота **немає** шляху видати `is_main`. Нова сторінка = ключ у `PAGE_KEYS` обох `roles.ts` + `UPDATE roles SET pages = pages || '["/шлях"]'` для потрібних ролей.
