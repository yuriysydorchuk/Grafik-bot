...
Тимчасова записка сесії (12–13.07.2026). Довговічне вже рознесено: команди тестів → `CLAUDE.md`
(розділ «Тести»); цей файл — що зробила сесія + що далі.

**Статус:** усе нижче **задеплоєно на прод** і працює (`healthz` = db ok / bot up). Кожен крок їхав
окремим комітом у `main` одразу після локальної верифікації (typecheck + 189/189 тестів + build).
Прод-рантайм за сесію змінили лише: security-хардилка (див. `HANDOFF-security-review.md`) і
1 баг-фікс (`DELETE /admins` FK-cleanup); решта — тести + тест-харнеси (у прод-бандл не входять).
Міграція одна: `2026-07-12-absence-shift-nullable.sql` (на проді no-op). Тестами знайдено **2 реальні
баги** (дрейф `schema.sql`, FK-delete адмінів) — обидва виправлені.
