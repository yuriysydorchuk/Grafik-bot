...
- **Статус:** ✅ У БОЮ — перша хвиля 1000 запущена 23.09.2026 о 10:00; задеплоєно 22.09.2026 (6e38b61): домен `https://espraca.pl` (A → 161.97.117.151, Caddy), `.env` з SMS-Fly, 3 міграції, кампанія «Хвиля 1 · вересень 2026» (draft, рекрутер Володимир id=10). Далі: тест-SMS з панелі, роль рекрутера (editData + сторінка), поповнення балансу, тест 300, хвиля
- **Дата:** 2026-09-21
...
**Перша хвиля (23.09.2026, 10:00):** кампанія «Хвиля 1 · вересень 2026», 13 283 у черзі, стеля тест-режиму 1000, батч 100/5 хв (≈50 хв на 1000), вікно вт/ср/чт 10:00–14:00. Черга сортується `year desc nulls last` — перші 1000 це 380 записів 2026 + 620 за 2025 (за id це були б 2019–2023 і конверсія занижена). Імпорт: з 15 052 рядків аркуша «Перевірені номери» додано 13 283 (пропущено 1 556 UA до 2024, 212 активних працівників, 1 дубль). Виявлено в бою: (1) SMS-Fly лімітує `GETMESSAGESTATUS` — по 10 паралельно давало 429 і доставка не підтягувалась → пачки по 3 з паузою 250 мс і ретраєм (aae3252); (2) `GET /api/sms-campaigns/NaN` з лінка рекрутації для кандидата без campaign_id → гард `router.param` + прихований лінк (ba1082b). Деплой цих фіксів — лише між батчами (рестарт посеред батча губить зарезервовані рядки).
