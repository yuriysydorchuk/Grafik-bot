...
- **Статус:** 🔄 в роботі — батч 1 зроблено 21.09 (схема, міграція, сервіси, крони, тести); далі батч 2
- **Дата:** 2026-09-21
...
- **Зроблено:** пропозиція узгоджена; батч 1 (схема+ядро+крони+тести) закомічено 21.09.
- **Лишилось:** батч 2 (публічна сторінка `/r/:token` + бот `?start=sms`), батч 3 (панель), батч 4 (автоматика/звіти), батч 5 (прод/ключі/тест 300).
