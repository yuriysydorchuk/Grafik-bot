...
## Дозакат 08.09.2026 ~20:10 — правки паралельної сесії з головного чекаута (dfc7705)

Після деплою Yuriy побачив «старі проблеми, які вже фіксив»: ті фікси лежали НЕЗАКОМІЧЕНИМИ у
робочому дереві головного чекаута (`/Users/yuriysydorchuk/PycharmProjects/Grafik-bot`, main на f87ca5e,
73 коміти позаду origin/main) і ніколи не були ні в git, ні на проді. Перенесено гілкою
`feature/leaver-grace` (патч ліг чисто, 736/736 тестів): грейс звільненого в боті (рапорт + години
60 днів), `payrollSummaries` не перетирає ручні джерела, реструктуризація доків (індекс API,
дворівнева автономія в CLAUDE.md, FINANCE_RULES.md, allow-list у settings.json). Міграцій немає.
`build.sh` → healthz ok/db ok/bot up.

Хвіст: у головному чекауті ті самі правки досі висять незакоміченими — перед `git pull` там треба
їх скинути (`git checkout -- . && git pull`), інакше конфлікт. Взаємодія з rehire (07.09): deep-link
фабрики обробляється раніше за гілку leaver, тож повернення через лінк працює як і було.

## Інцидент після деплою 08.09 ~20:30 — сповіщення знову приходили ролям, де їх вимкнули

Причина: на кроці міграцій я прогнав УСІ файли від 2026-08-28, включно з уже накоченими на проді
main-міграціями. `2026-09-01-role-notify-prefs.sql` містить `UPDATE roles SET notify = <повний список>
WHERE notify = '[]'` — «порожньо» там означало не «не ініціалізовано», а «власник вимкнув усе»
(роль OnTon). Повторний прогін увімкнув усе назад; `tasks-module.sql` дописав ще тип `tasks`
(OnTon, бухгалтерія). Решта ролей збігалась із бекапом (+ `tasks` — за задумом).
Виправлено з нічного бекапу `/root/backups/db-2026-09-08.sql.gz`: `onton → []`,
`buhgalteria → ["finance_alerts"]`. Інші перезапущені файли шкоди не зробили (email-отримувачі —
NOT EXISTS + unique index; vat_rate — нормалізація).

Урок (перенесено в DEPLOYMENT.md і CLAUDE.md): накочувати лише НОВІ файли міграцій (яких ще не було
на проді), а не «усе від дати»; `UPDATE … WHERE col = default` — одноразові й НЕ ідемпотентні.
