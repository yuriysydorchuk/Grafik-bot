...
- **Немає міграцій-файлів.** Схема й БД синхронізуються вручну (`psql`) — легко розсинхронити.
  `deploy/schema.sql` тримати актуальним (перегенеровувати після змін схеми — див. [DATABASE.md](DATABASE.md)).
- **`uploads/` поза git.** Не входить у `git pull` і в дамп БД — потрібен окремий бекап, інакше
  файли документів втрачаються при переустановці.
- **Бекапи app-рівня НЕ налаштовані** (станом на 2026-06-19: crontab порожній, дампів немає).
  Ймовірно увімкнено Contabo Auto Backup (снапшоти VM) — але це не гарантує консистентний дамп БД.
