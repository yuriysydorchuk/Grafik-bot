...
**Графік:** `GET /weeks`, `GET /schedule` (віддає і `shiftOverrides` — разові зміни тижня), `GET /schedule/excel`, `POST /schedule/generate` (`augment: true` = «доповнити»: наявні записи не видаляються, заповнюються лише порожні клітинки), `POST /schedule/approve`, `POST/DELETE /schedule/shift-override` (разова зміна на конкретну дату фабрики: № зміни + start/end; правлять `hoursOverride` явок клітинки), `POST /schedule/email` (лист клієнту фабрики: тиждень або день, з Excel-вкладенням), `GET/PUT /email-templates` (шаблон листа, ключі `email_tpl_schedule_*` у `settings`), `POST /schedule/entry` (створює draft-тиждень, якщо графік ще не генерувався — ручне збирання drag-ом працює до «Згенерувати»), `POST /schedule/copy-day` (кнопка «📋 На тиждень»: людей джерельного дня в ті самі зміни вибраних днів; `mode=preview` — класифікація ok / без диспозиційності / відпросився / пропуск, `mode=apply` + `force` — вставка з копією разового часу зміни й `hoursOverride`; під інтеграційними тестами), `PATCH/DELETE /schedule/entry/:id`, `PATCH /schedule/entry/:id/status`, `PUT /schedule/driver-assignments(/by-driver)`, `POST /schedule/driver-assignments/copy-week`, `POST /schedule/notify(-workers|-driver|-drivers)`, `POST /schedule/shift-cancel|shift-restore` (скасування клітинки день+зміна: знімає призначення водіїв, скидає absent, опційні сповіщення працівникам/водіям; entries лишаються scheduled → поза reliability; бот/пуші/pickup-прогалини пропускають скасовані клітинки)

**Облік/відсутності:** `GET /hours` (рядки по парі працівник+фабрика); з cap `svodni` рядок несе `unlegalized` (без форми легалізації і не студент, або oczekuje) — червона підсвітка в Обліку годин, дзеркало бейджа «?» сводної, `POST /hours/report` (ручні години рапорту), `POST /hours/report-remind` (нагадування по парах працівник×фабрика: людині на 2-3 фабриках місяця перелічуються фабрики, яких бракує; активні без явок місяця не отримують; бот після збереження рапорту сам пропонує решту фабрик inline-кнопками), `GET /hours/report-excel` (`cols=` вибір колонок з каталогу `HOURS_XLSX_COLS`, `errorsOnly=1` — лише розбіжності рапорт↔фабрика, підтверджені «все ок» пропускаються), **години з фабрики** (`factory_hours`, unique worker+month+factory; `days` jsonb: число = разом за день АБО `{№зміни: год}` — позмінна евіденція): `POST /hours/factory-parse` (Excel 4 формати — матриця «Razem», lista dni szczegółowo, ewidencja I/II/III ANDROS (аркуші KOBIETY+MĘŻCZYZNI зливаються, місяць з імені файла), розрахунковий файл Eurocash (`Nazwisko i Imię … Stawka H NEW AGENCJA`; крім годин парсяться extras: нічні/продуктивність/ставка агенції/потроненя → `factory_hours.extras` jsonb) — або вставлений текст, рядки можуть нести ключ фабрики «ключ; ім'я; години»; матчинг: спершу ключ фабрики (`worker_factory_codes`, порівняння без провідних нулів) → впевнений збіг без fuzzy, далі bot/workerMatch з перевагою «вже в обліку місяця» при дублях), `POST /hours/factory-apply` (upsert; `create:true` рядки створюють профілі — `workers.created_source='hours_import'`, фірма з `createCompanyId`; такі профілі можна 🗑 зі списку обліку), `POST /hours/factory` (одне значення або повна `days`-заміна, підсумок = Σ днів), `POST /hours/factory-clear` (очистка колонки вкладки по workerIds), `POST /hours/factory-confirm` (галочка «все ок»: розбіжність або години без рапорту → зелений; скидається при зміні годин), `POST /hours/factory-notify` (розсилка «підтверди свої години» в бот 5 мовами, кнопки ✅/❌; відповідь+пояснення в `factory_hours.worker_response/worker_note`, колонка «Підтв. працівника»; вкладка обліку — завжди ОДНА на фабрику (фабрика шле одну евіденцію на всіх; `factories.multi_firm` ділить лише сводну from-hours, рішення 06.08.2026)), **ключі фабрики** (`worker_factory_codes`: особистий номер працівника в системі фабрики; factory-apply сам запам'ятовує пари з підтверджених рядків із ключем, вкл. Nr Osobowy Eurocash): `GET /hours/factory-codes?factoryId`, `POST /hours/factory-codes-parse` (превʼю списку «ключ; ім'я», матчинг workerMatch), `POST /hours/factory-codes-apply` (upsert; ключ, зайнятий іншим профілем, — у `conflicts`), `DELETE /hours/factory-codes/:id`; веб — кнопка «🔑 Ключі» на вкладці фабрики в Обліку годин, **склад списку місяця**: нульові рядки активних — лише від дати працевлаштування (нема → дата створення профілю); `POST /hours/exclusions` + `POST /hours/exclusions-remove` (приховати/повернути людину в місяці: manual/vacation/not_started, таблиця `hours_month_exclusions`; реальні дані місяця повертають рядок самі; GET /hours віддає `excluded` → чип «приховано N» на вкладці), `POST /hours/last-activity` (остання дата даних людей — попередження про конфлікт при звільненні ранішою датою); веб: мультивибір рядків (чекбокси) → «Прибрати зі списку місяця» / «Звільнити…» (дата «від коли»), після імпорту годин — модалка «залишків» вкладки (без годин/рапорту/явок: залишити / звільнити / відпустка / ще не приступив + масові кнопки), `GET /worker-days/:id`, `POST /worker-days/:id/add-shift`, `PATCH /worker-days/entry/:id`, `GET /absences` (пропуски + штрафи: стандарт 200 zł, `justified` виключає з підсумків), `PATCH /absences/:entryId` (виправдання/штраф пропуску), `GET /absence-requests`, `POST /absence-requests/:id/approve|reject|substitute` (reject приймає опційну `reason` — зберігається в `reject_reason` і додається у сповіщення працівнику), `GET /hours-reports`, `GET /hours-reports/:id/photo`, `POST /hours-reports/:id/apply|resolve`, `GET /reliability`, `GET /trips`, `POST /unplanned/:id/link` (привʼязка вписаного водієм позапланового до працівника з бази; створює явку present)

**Аванси (залічки):** `GET /advances` (+ місто, фірма працівника, IBAN, хто подав/підтвердив), `POST /advances` (офісна подача — одразу approved=«передано до виплати» від імені подавача), `POST /advances/:id/approve|reject|paid` (`editData`; запит працівник створює в боті; `paid` приймає `method` transfer|cash — ручна виплата вимагає вибору, дата = день кліку), `PATCH /advances/:id` (перенесення групи виплат, лише approved). **Група виплати** ставиться при затвердженні за датою Warsaw: 1–14 → «15», 15–29 → «30», 30–31 → «15» наступного місяця (`lib/advancePayout.ts`, під тестами). Авто-«виплачено» по банку (`services/advances.ts`) пише `paid_method='transfer'`. **Рахунки працівника:** `GET/POST /workers/:id/bank-accounts`, `POST /worker-bank-accounts/:id/primary` (основний = «номер рахунку» профілю: показ/копіювання в авансах, майбутній файл виплат; перший ручний стає основним сам), `DELETE /worker-bank-accounts/:id`
...
| `0 6 * * *` | Імпорт банківських витягів MT940 з Drive → `bank_transactions`/`bank_statements` (ідемпотентний, `services/bankStatements.ts`) + фактури витрат, зведені ЗП, KSeF-синк (вкл. `matchKsefPayments`), контрагенти, авто-«виплачено» авансів, попередження про сплив консентів. Каса зі STAN KASY **більше не синкається** (таблиця виведена 08.2026) |
