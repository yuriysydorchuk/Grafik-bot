...
- **Класифікація витягів живе ЛИШЕ в `routes/bank.ts`** (SQL-патерни: bucket-и + `EXPENSE_CATS`, перший збіг виграє). У Postgres межа слова — `\y`, **не** `\b` (тихо не матчиться); польські відміни (bankomat/bankoma**cie**) — матчити корінь. Кредитний рахунок `PL75…8415` виключений з операційної каси (борг). Нова юрособа/назва підпапки на Drive = правка `matchCompanyName` у `bankStatements.ts`.
- **`xlsx` (SheetJS) 0.18.5 — відомі CVE, свідомо не пофікшено (прийнятий залишковий ризик).** Prototype pollution (CVE-2023-30533, фікс 0.19.3) + ReDoS (CVE-2024-22363, фікс 0.20.2) на **читанні** (`XLSX.read`/`sheet_to_json`); фікси є **лише на CDN SheetJS**, не в npm. Поверхня в нас обмежена: обидва `XLSX.read` у боті (`bot/index.ts` — імпорт графіка/кандидатів) **за гейтом `isAdmin`**, у `services/drive.ts` читаються лише власні файли з Drive; `XLSX.write` не вражений. Ризик: отруєний файл від скомпрометованого/обдуреного адміна; ReDoS кладе весь одно-процесний застосунок. **Коли робитимемо фікс — варіант 1:** пін CDN-тарбола `"xlsx": "https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz"` (API той самий, правок коду нема). Альтернатива — міграція на наявний `exceljs` (але він не читає старий бінарний `.xls`).
